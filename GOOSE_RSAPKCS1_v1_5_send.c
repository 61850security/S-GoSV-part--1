...
char length_2=0x91;
char resrv1_1=0x00; 		     
char resrv1_2=0x86; /* the length of the hash generated is 128 + extension field length 6 bytes)=134 bytes */                  
char resrv2_1=0xF2; 		     
char resrv2_2=0x32;             
...
/* Extension field */ 
char SEQUENCE_tag=0x30; // Sec Extention Tag. 1 byte 0x30
char SEQUENCE_length=0x84; //length of the extension 132 => 0x84
char Auth_value_tag=0xA4;  // Authentication value tag 0xA4. 
char Auth_length=0x82;  // 128 + 2 = 130 in hex 0x82
char mac_tag=0x85;
char mac_length=0x80;   // 128 in decimal , 0x80 in hexadecimal

unsigned char hash_val[128]= 
	{
		
                
	 }; 
...
int main(int argc, char *argv[])
{
	...
	/* Extension fields */
	sendbuf[tx_len++] = SEQUENCE_tag; 
	sendbuf[tx_len++] = SEQUENCE_length; 
	sendbuf[tx_len++] = Auth_value_tag; 
	sendbuf[tx_len++] = Auth_length;  
	sendbuf[tx_len++] = mac_tag;
	sendbuf[tx_len++] = mac_length;


	/*  HMAC Digest  */
  	for( i=0; i<128; i++)
        	sendbuf[tx_len++] = hash_val[i];
        ...
        if (sendto(sfd, sendbuf, tx_len, 0, (struct sockaddr*)&socket_address, sizeof(struct sockaddr_ll)) < 0)
            ...
            printf("\n GOOSE Message values with security Sent :\n");
            for (i=0; i < tx_len; i++)
